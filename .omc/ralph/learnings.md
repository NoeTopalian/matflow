...
iter-4 | NEW (Sparkline mobile overflow) | Stats reports page Sparkline component renders an SVG with hardcoded width=520 and viewBox=520x150. The SVG bypasses the existing ResponsiveContainer pattern used elsewhere in ReportsView. On mobile (375px viewport) the chart overflows the page and triggers horizontal body scroll. Fix: wrap the Sparkline consumer at ReportsView.tsx:394 in a w-full overflow-hidden container with the Tailwind arbitrary selector `[&>svg]:w-full [&>svg]:h-auto` so the SVG scales while preserving its viewBox aspect ratio. Pattern reusable for any other fixed-width Sparkline/chart consumer in the codebase. Note: shipped without a Playwright regression test because the e2e auth setup for the reports route is heavy; reviewer's check 3 deferred with explicit acknowledgment in the commit. To backfill: add `tests/e2e/reports-mobile.spec.ts` once a shared owner-auth helper exists.
