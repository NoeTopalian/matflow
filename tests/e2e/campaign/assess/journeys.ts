...
export const JOURNEYS: readonly Journey[] = [
  ...
  { id: "J63", name: "Attribution funnel: a taster credited to a coach converts and is counted", group: "L-B", primaryRole: "owner", allowed: OM, na: [...NOT_TENANT, "anonymous"], routes: ["GET attribution", "POST members", "PATCH members/[id]", "/dashboard/attribution"], note: "M1. Staff create a taster with trialRunById/creditedToUserId, PATCH to active; a MemberStatusEvent trail null→taster→active (reason staff_edit) is written by the single-writer lib/member-status.ts; GET attribution counts trials/sign-ups/conversions per coach with a min-N guard (<3 trials → no %), a feature-epoch window, and import-reason events excluded. Webhook + importer routing is fast-follow." },
  { id: "J64", name: "Owner mints, rotates and disables the leaderboard display URL", group: "L-B", primaryRole: "owner", allowed: ["owner"], na: [...NOT_TENANT, "anonymous"], routes: ["GET settings/display", "POST settings/display"], note: "M4. Mirrors the kiosk token panel: enable 409s when already enabled, regenerate rotates the hash, disable is a POST action; the raw display token is returned once and only the hash is stored. A SEPARATE credential from the kiosk token." },
  { id: "J65", name: "A club's short URL /[slug] redirects to its branded login; reserved segments 404", group: "L-B", primaryRole: "anonymous", allowed: ["anonymous"], na: [...ACCOUNTS, ...NOT_TENANT], routes: ["/[slug]"], note: "M3 (superseded 2026-09-23 per Noe): /[slug] no longer renders a public page — it redirects to /login?club=<slug> (the branded sign-in), skipping the club-code step. No DB lookup here, so a real/unknown/suspended club are indistinguishable at this route (stronger than before); branding + the honest paused message are decided downstream by the login page + GET tenant/[slug]. A reserved slug (a real route folder) 404s via isReservedSlug." },
  { id: "J66", name: "Public leaderboard board: ranks correct, opt-out absent, no PII, paused gated", group: "L-B", primaryRole: "anonymous", allowed: ["anonymous"], na: [...ACCOUNTS, ...NOT_TENANT], routes: ["/leaderboard/[token]"], note: "M4. Recompute-on-read groupBy; payload is {rank,name,checkIns,movement} only — first name + last initial, no ids/emails/photos; leaderboardOptOut excluded entirely; a paused club shows the paused screen not 404; a wrong or short token 404s." },
];
