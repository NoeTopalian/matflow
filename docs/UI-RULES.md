...
- **Warning text does not sit on the zebra row.** `--hue-warning-ink` reads 4.34:1 on `--sf-2` — the one ink that does not clear the floor everywhere. Put it on white or on its own warm tint. The `--cat-*` action-item accents follow the same discipline: `--cat-admin` is a darker amber rather than an alias of `--hue-warning-ink`, precisely because those rows sit on `--sf-2`.
- Fix on sight (known live bugs): none currently tracked. Fixed 2026-08-17: Geist variables moved to `<html>` (they sat on `<body>` while `font-sans` resolved at `<html>`, so the entire app rendered in the browser's default serif — Times New Roman, verified live against matflow.studio/login); the duplicated `darkTheme` token block in `app/dashboard/layout.tsx`; white-alpha scrollbars/`.skeleton`/`.glass` in globals.css; self-referential `--font-sans`. The light text ramp was recalibrated twice — the second pass MEASURED it and supersedes the first, which had been chosen by eye and left `--tx-3` at 3.4:1.
